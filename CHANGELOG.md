...
- **대화가 길어질수록 앱이 무거워지던 것을 고쳤습니다.** 실행 하나에 활동 기록이 수천 줄까지
  쌓이는데 화면이 그걸 전부 그리고 있었고, 끝난 작업 블록이 접히지도 않아 그 줄들이 대화
  내내 남아 있었습니다. 이제 끝난 작업은 한 줄로 접히고, 펼쳐도 최근 것부터 보여 준 뒤
  "이전 N개 더 보기"로 이어집니다. 기록은 하나도 지우지 않습니다.

### 목표(Goal)

- **목표를 켜면 진행도 되고 닫히기도 합니다.** 목표를 켜면 번호만 찍히고 정작 그 목표의
  계약이 만들어지지 않아, 화면에는 목표 칩이 떠 있는데 닫을 것도 이어갈 것도 없는 상태가
  됐습니다. 이제 목표를 켜는 그 자리에서 계약도 함께 기록합니다.
- 목표가 취소로 끝났을 때 화면이 그 사실을 못 받아 칩이 계속 켜져 있던 것도 고쳤습니다.
- **오래 걸리는 작업이 일을 다 해놓고 "확인할 수 없음"으로 멈추던 것을 고쳤습니다.** 무엇을
  했는지 확인하는 쪽에 그 기록이 너무 길다는 이유로 잘려서 전달되고 있었고, 잘린 자리가
  하필 기록 한가운데라 아예 읽히지 않았습니다. 이제 기록이 아무리 길어도 요약과 함께
  온전히 전달되고, 명령의 **마지막 줄**(테스트 통과 여부 같은 결론)이 항상 포함됩니다.

### 대화 화면

- **모든 말풍선이 필요한 폭보다 12% 좁던 것을 고쳤습니다.** 짧은 글은 글자가 두 줄로
  쪼개지고 긴 문장은 한 줄이 더 흘렀습니다.
- **"건너뛰기"를 눌렀는데 적어 둔 답이 전송되던 것을 고쳤습니다.** 이제 적어 둔 답이 있으면
  단추가 "이 답 보내기"로 바뀝니다.
- 결과 목록이 없는 파일을 "아직 읽지 않았습니다"라고 말하거나, 실패한 명령이 만들려던 파일을
  결과물로 세거나, 읽은 내용이 화면에 도달하지 않던 것들을 고쳤습니다.
- 제품 메뉴에서 Science를 눌러도 아무 일이 없던 것, 지울 수 없는 항목이 이유 없이 안 눌리던
  것을 고쳤습니다.
- 한국어 화면에 영어로 남아 있던 문구들을 고치고, 다시 섞이지 않도록 기계로 훑습니다.

### 화면을 쓰기 어렵던 자리

- **키보드로 옮겨 다닐 때 지금 어디에 있는지 안 보이던 입력칸들을 고쳤습니다.** 화면 30개를
  훑어 남은 곳이 없게 했습니다.
- **마우스를 올려도 아무 반응이 없던 단추들**과, **회색인데 왜 못 누르는지 말해 주지 않던
  단추들**을 고쳤습니다.
- **Escape 로 닫히지 않던 창들과, 닫은 뒤 위치를 잃던 자리들**을 고쳤습니다. 화면을 덮는데
  닫는 방법이 아예 없던 창도 있었습니다.
- **사람이 읽을 말 대신 내부 표시가 그대로 뜨던 자리 26곳**을 사람의 말로 바꿨습니다.
- **읽어 오지 못했을 때 "없음"이라고 말하던 자리들**을 고쳤습니다. 프로젝트가 있는데
  "첫 프로젝트를 연결하세요"라고 하던 것이 그중 하나입니다.
- **보냈는데 시작되지 않으면 쓴 글이 사라지던 것**을 고쳤습니다. 이제 글을 되돌려 주고
  이유를 말합니다.
- 결제 승인을 묻는 창이 영어로만 뜨던 것, 좁은 창에서 이름이 잘리는데 전문을 볼 곳이
  없던 것도 고쳤습니다.

### Science

- 연구가 중간에 멈추면 다시 시작되지 않던 것, 논문 근거로 쓸 자료가 전부 거절되던 것,
  이미 받아 둔 자료 145건이 못 쓰이던 것을 고쳤습니다.
- "내용을 불러오는 중"이 끝나지 않던 화면들과, "CSV 가져오기"가 검증 중에서 멈추던 것을
  고쳤습니다. 파일 선택창이 안 열리는 환경을 위해 경로를 직접 넣는 길을 냈습니다.
- 사전 등록한 분석 계획의 잠금은 **그대로 둡니다.** 결과를 본 뒤 계획을 바꾸는 것을 막는
  장치이고, 실제로 그것을 우회하려던 수리를 되돌렸습니다.
...
### Science — 연구가 스스로 굴러갑니다

- **처음 쓰는 사람도 연구를 시작할 수 있습니다.** Science 는 자기 모델 선택을 갖는데, 그것이
  "직전에 고른 것"으로만 구현돼 있어 한 번도 고른 적 없는 기계에서는 연구가 시작조차 되지
  않았습니다. 앱이 이미 찾아 둔 런타임으로 처음 한 번을 정하고, 그 뒤로는 연구자의 선택입니다.
- **승인해 둔 연구가 승인 앞에서 멈추지 않습니다.** 프로젝트는 기본으로 자율 진행 권한을
  갖는데, 도구 설명이 "여기서는 거부된다"고만 적고 그 권한을 말하지 않아, 연구가 열려 있는 문
  앞에서 사람을 기다렸습니다.
- **긴 연구 요청이 더 이상 시작 전에 죽지 않습니다.** 2,000자를 넘는 지시가 본문 검색 한도에
  걸려 모델이 돌기도 전에 턴이 끝났습니다.
- **프로젝트 이름이 질문 앞부분 잘라내기가 아니라 제목입니다.** 첫 문장에서 요청 어투를 걷고,
  길면 첫 절로 이름을 짓습니다.

### Science — 화면

- **모든 목적지가 같은 자리에서 시작합니다.** 본문 좌우 여백·읽는 폭·제목 크기가 화면마다
  달랐습니다(폭 760/820/860, 제목 25/26/28px). 하나로 맞췄습니다.
- **왼쪽 메뉴에서 결과물이 먼저 사라지지 않습니다.** 12개 중 넓은 창에서 10개만 보였고, 항상
  결과·원고·제출부터 잘렸습니다. 이제 전부 보이고, 좁은 창에서도 지금 보고 있는 화면은 늘
  표시됩니다.
- **데이터 표가 진짜 표가 됐습니다.** 100행씩 넘기던 표 대신 정렬·필터·열 조절이 되는
  가상 스크롤 표(Tabulator, MIT)를 씁니다. 결측과 수식형 셀 표기는 그대로입니다.
- 읽기 어려운 회색 글자(대비 미달) 61곳과 포커스가 안 보이던 입력 4곳을 고쳤습니다.

### 진단

- 실패 원문에서 비밀값을 가리는 규칙이 낱말 안쪽을 열쇠로 오인해 오류 코드를 지우고 있었습니다.
  그래서 자율 연구가 왜 멈췄는지 알 수 없었습니다.
...
Includes the Science, Work, Graph, and Hub fixes below. Corrects the deep
light-curve analysis tool's documented column inputs and fits verified result
images inside the Science research panel. Release checks use authenticated
GitHub metadata requests. The 1.1.6 installer build was not published.

This release binds Agentlas OS v1.2.44 at 1f6d64374502cfd5f8581ad3c1fb18691ed61b1d.
Its public runtime asset `hephaestus-runtime-v1.2.44.tar.gz` is pinned at SHA-256 `795d1c294db662475d4eb0c7e4562ba0ca1e0f27d3b1da0f5e9c6e7cf627d83a`.
Source readiness does not prove a public installer or update feed; the Releases page stays the authority.

## 1.1.6 — 2026-09-07 (installer build not published)
