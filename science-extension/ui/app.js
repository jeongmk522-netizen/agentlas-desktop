  ...
  root.addEventListener("submit", async (event) => {
    ...
    if (event.target.id === "submission-export-form") {
      ...
      if (profile.status !== "verified") {
        state.journalActionError = uiCopy("저널 지침에 아직 확인되지 않은 항목이 있습니다. 저널 규칙을 확인한 뒤 다시 검증해 주세요. 제출 파일은 생성되지 않았습니다.", "Some journal guidelines still need review. Review the journal rules before validating again. No submission file was created.");
        render();
        document.querySelector("#submission-export-form .formError")?.scrollIntoView({ block: "nearest" });
        return;
      }
      const form = new FormData(event.target);
